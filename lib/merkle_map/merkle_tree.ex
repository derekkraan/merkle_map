defmodule MerkleMap.MerkleTree do
  ...
  def diff_keys([], [], _loc), do: []

  def diff_keys({h, _, _}, {h, _, _}, _loc) do
    []
  end

  def diff_keys([], {_, _, _} = tree, loc),
  ...
  def diff_keys([], {_, values}, _loc), do: Map.keys(values)
  ...
  defp init_empty_leaf([]), do: {[], %{}}

  defp init_empty_inner_node({_hash, _a, _b} = tree), do: tree
  defp init_empty_inner_node([]), do: {[], [], []}
end
